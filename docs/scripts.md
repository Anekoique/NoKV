...
### `tools/scripts/run_local_cluster.sh`
...
- **Usage**
  ```bash
  ./tools/scripts/run_local_cluster.sh --config ./raft_config.example.json --workdir ./artifacts/cluster
  ```
...
### `tools/scripts/bootstrap_from_config.sh`
...
- **Usage**
  ```bash
  ./tools/scripts/bootstrap_from_config.sh --config /etc/nokv/raft_config.json --path-template /data/store-{id}
  ```
...
### `tools/scripts/serve_from_config.sh`
...
| Script | Purpose |
| --- | --- |
| `tools/scripts/recovery_scenarios.sh` | Runs crash-recovery scenarios across WAL/manifest/vlog. Set `RECOVERY_TRACE_METRICS=1` to collect metrics under `artifacts/recovery/`. |
| `tools/scripts/transport_chaos.sh` | Injects disconnects/blocks/delay into the `raftstore` transport to observe behaviour under faulty networks. |
| `tools/scripts/run_benchmarks.sh` | Executes YCSB benchmarks (default engines: NoKV/Badger/Pebble, workloads A-G; optional RocksDB via build tags). |
| `tools/scripts/debug.sh` | Convenience wrapper around `dlv test` for targeted debugging. |
| `tools/scripts/gen.sh` | Generates protobuf Go bindings through Buf with pinned remote plugin versions. |
...
- Go tools or custom scripts can import `github.com/feichai0017/NoKV/cluster/config` and call `config.LoadFile` / `Validate` to consume the same `raft_config.json`, avoiding divergent schemas.
