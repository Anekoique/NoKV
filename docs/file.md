...
| --- | --- | --- |
| [`Options`](../storage/file/file.go#L5-L16) | Parameter bag for opening files (FID, path, size). | Used by WAL/vlog managers. |
| [`MmapFile`](../storage/file/mmap_linux.go#L12-L98) | Cross-platform mmap wrapper. | `OpenMmapFile`, `AppendBuffer`, `Truncate`, `Sync`. |
| [`LogFile`](../storage/file/vlog.go#L17-L223) | Value-log specific helper built on `MmapFile`. | `Open`, `Write`, `Read`, `DoneWriting`, `Truncate`, `Bootstrap`. |
...
While SSTable builders/readers live under `storage/lsm/table.go`, they rely on `file` helpers to map index/data blocks efficiently. The build tags (`sstable_linux.go`, `sstable_darwin.go`) provide OS-specific tuning for direct I/O hints or mmap flags.
...
By keeping all filesystem primitives in one package, NoKV ensures WAL, vlog, and SST layers share consistent behaviour (sync semantics, truncation rules) and simplifies testing (`storage/file/mmap_linux_test.go`).
