...
- [`storage/vlog/manager.go`](../storage/vlog/manager.go) – owns the open file set, rotation, and segment lifecycle helpers.
- [`storage/vlog/io.go`](../storage/vlog/io.go) – append/read/iterate/verify/sample IO paths.
- [`engine/vlog.go`](../engine/vlog.go) – integrates the manager with the DB write path, discard statistics, and garbage collection (GC).
...
- **DB policy layer (`engine/vlog.go`, `engine/vlog_gc.go`)** – integrates the manager with
  the DB write path, persists vlog metadata in the manifest, and drives GC
  scheduling based on discard stats.
- **Value-log manager (`storage/vlog/`)** – owns segment lifecycle (open/rotate/remove),
  encodes/decodes entries, and exposes append/read/sample APIs without touching
  MVCC or LSM policy.
- **File IO (`storage/file/`)** – mmap-backed `LogFile` primitives (open/close/truncate,
...
* Files are named `%05d.vlog` and live under `workdir/vlog/bucket-XXX/` when `Options.ValueLogBucketCount > 1`. [`Manager.populate`](../storage/vlog/manager.go) discovers existing segments at open.
* `Manager` tracks the active file ID (`activeID`) and byte offset; [`Manager.Head`](../storage/vlog/manager.go) exposes these so the manifest can checkpoint them (`manifest.EditValueLogHead`).
* Files created after a crash but never linked in the manifest are removed during [`valueLog.reconcileManifest`](../engine/vlog.go).
...
* `vlog.VerifyDir` scans all segments with [`sanitizeValueLog`](../storage/vlog/io.go) to trim corrupted tails after crashes, mirroring RocksDB's `blob_file::Sanitize`. Badger performs a similar truncation pass at startup.
...
1. **Append + Rotate** – [`Manager.AppendEntry`](../storage/vlog/io.go) encodes and appends into the active file. The reservation path handles rotation when the active segment would exceed `MaxSize`; manual rotation is rare.
2. **Crash recovery** – [`Manager.Rewind`](../storage/vlog/manager.go) truncates the active file and removes newer files when a write batch fails mid-flight. `valueLog.write` uses this to guarantee idempotent WAL/value log ordering.
3. **Safe reads** – [`Manager.Read`](../storage/vlog/io.go) returns an mmap-backed slice plus an unlock callback. Active segments take a per-file `RWMutex`, while sealed segments use a pin/unpin path to avoid long-held locks; callers that need ownership should copy the bytes before releasing the lock.
4. **Verification** – [`VerifyDir`](../storage/vlog/io.go) validates entire directories (used by CLI and recovery) by parsing headers and CRCs.
...
1. [`valueLog.write`](../engine/vlog.go) builds a write mask for each batch, then delegates to [`Manager.AppendEntries`](../storage/vlog/io.go). Entries staying in LSM (`shouldWriteValueToLSM`) receive zero-value pointers.
...
3. Any error triggers `Manager.Rewind` back to the saved head pointer, removing new files and truncating partial bytes. [`engine/vlog_test.go`](../engine/vlog_test.go) exercises both append- and rotate-failure paths.
...
* `lfDiscardStats` aggregates per-file discard counts from LSM compaction workers (`storage/lsm/executor.go`, `subcompact` -> `updateDiscardStats`). Once the in-memory counter crosses [`discardStatsFlushThreshold`](../engine/vlog.go#L27), it marshals the map into JSON and writes it back through the DB pipeline under the special key `!NoKV!discard`.
...
* GC uses `discardRatio = discardedBytes/totalBytes` derived from [`Manager.Sample`](../storage/vlog/io.go), which applies windowed iteration based on configurable ratios. If a file exceeds the configured threshold, [`valueLog.doRunGC`](../engine/vlog_gc.go) rewrites live entries into the current head via `db.batchSet` (the normal commit pipeline) and then [`valueLog.rewrite`](../engine/vlog_gc.go) triggers manifest delete edits through `removeValueLogFile`.
...
2. [`valueLog.open`](../engine/vlog.go) launches `flushDiscardStats` and iterates every vlog file via [`valueLog.replayLog`](../engine/vlog.go). Files marked invalid in the manifest are removed; valid ones are registered in the manager's file map.
...
* Metrics in [`engine/stats.go`](../engine/stats.go) report segment counts, pending deletions, discard queue depth, and GC head pointer via `expvar`.
...
* `nokv vlog --workdir <dir>` loads a manager in read-only mode and prints current head plus file status (valid, gc candidate). It invokes [`vlog.VerifyDir`](../storage/vlog/io.go) before describing segments.
