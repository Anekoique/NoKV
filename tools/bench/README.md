...
benchmark script (`tools/scripts/run_benchmarks.sh`).
...
tools/scripts/run_benchmarks.sh
  -> go test ./tools/bench -run TestBenchmarkYCSB -args <flags>
     -> TestBenchmarkYCSB (tools/bench/ycsb_test.go)
        -> runYCSBBenchmarks (tools/bench/ycsb_runner.go)
           -> engine.Open(clean)
...
- Engine interface: `tools/bench/ycsb_engine.go` defines `Read/Insert/Update/Scan`
  and per-engine implementations live in `tools/bench/ycsb_engine_*` (including
  `nokv-skiplist` / `nokv-art` for memtable-only comparisons).
- Workload model: `tools/bench/ycsb_runner.go` defines YCSB A/B/C/D/E/F/G mixes,
